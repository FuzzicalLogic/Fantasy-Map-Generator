...
function zonesEditorAddLines() {
    ...
    zones.selectAll("g").each(function () {
        ...
        const rural = d3.sum(c.map(i => pack.cells[i].pop)) * populationRate.value;
        const urban = d3.sum(c.map(i => pack.cells[i].burg).map(b => pack.burgs[b].population)) * populationRate.value * urbanization.value;
        ...
        <input data-tip="Zone description. Click and type to change" class="religionName" value="${description}" autocomplete="off" spellcheck="false">
    ...
    const totalPop = (d3.sum(pack.cells.map(x => x.pop)) + d3.sum(pack.burgs.filter(b => !b.removed).map(b => b.population)) * urbanization.value) * populationRate.value;
...
function selectZoneOnMapClick() {
    ...
    const el = getBody().querySelector("div[data-id='" + zone + "']");
    selectZone(el);
}
...
function addZonesLayer() {
    ...
    getBody().insertAdjacentHTML("beforeend", line);
    zonesFooterNumber.innerHTML = zones.selectAll("g").size();
}
