...
const getBody = () => getById('provincesBodySection');
...
export function editProvinces() {
    ...
    body.addEventListener("click", function (ev) {
        ...
        if (cl.contains("fillRect"))
            ...
            provinceOpenCOA(ev, p);
        else if (cl.contains("icon-star-empty"))
            capitalZoomIn(p);
        else if (cl.contains("icon-flag-empty"))
            triggerIndependencePromps(p);
    ...
    body.addEventListener("change", function (ev) {
        const el = ev.target, cl = el.classList, line = el.parentNode, p = +line.dataset.id;
        if (cl.contains("cultureBase"))
            changeCapital(p, line, el.value);
    });
}
...
function provincesEditorAddLines() {
    ...
    if (selectedState != -1)
        filtered = filtered.filter(p => p.state === selectedState); // filtered by state
    body.innerHTML = "";
...
function triggerIndependencePromps(p) {
    ...
    $("#alert").dialog({
        resizable: false, title: "Declare independence",
        buttons: {
            Declare: function () {
...
function declareProvinceIndependence(p) {
    ...
    const diplomacy = states.map(s => {
        ...
        if (s.i === oldState)
            relations = "Enemy"; // new state is Enemy to its old overlord
        else if (relations === "Ally")
            relations = "Suspicion";
        else if (relations === "Friendly")
            relations = "Suspicion";
        else if (relations === "Suspicion")
            relations = "Neutral";
        else if (relations === "Enemy")
            relations = "Friendly";
        else if (relations === "Rival")
            relations = "Friendly";
        else if (relations === "Vassal")
            relations = "Suspicion";
        else if (relations === "Suzerain")
            relations = "Enemy";
    ...
    if (states[oldState].provinces.includes(p))
        states[oldState].provinces.splice(states[oldState].provinces.indexOf(p), 1);
...
function removeProvince(p) {
    ...
    $("#alert").dialog({
        resizable: false, title: "Remove province",
        buttons: {
            Remove: function () {
                pack.cells.province.forEach((province, i) => {
                    if (province === p)
                        pack.cells.province[i] = 0;
                });
                const state = pack.provinces[p].state;
                if (pack.states[state].provinces.includes(p))
                    pack.states[state].provinces.splice(pack.states[state].provinces.indexOf(p), 1);
...
function editProvinceName(province) {
    ...
    function addCustomForm() {
        ...
        if (displayed)
            applyOption(provinceNameEditorSelectForm, value);
    }
...
function changeForSelection(selection) {
    ...
    selection.forEach(i => {
        ...
        if (i === pack.provinces[provinceOld].center) {
            ...
            if (!center.size())
                centers.append("polygon").attr("data-center", i).attr("points", getPackPolygon(i));
        ...
        if (exists.size()) {
            if (pack.cells.province[i] === provinceNew)
                exists.remove();
...
function removeAllProvinces() {
    ...
    $("#alert").dialog({
        resizable: false, title: "Remove all provinces",
        buttons: {
            Remove: function () {
                $(this).dialog("close");
...
function closeProvincesEditor() {
    if (customization === 11)
        exitProvincesManualAssignment("close");
    if (customization === 12)
        exitAddProvinceMode();
}
