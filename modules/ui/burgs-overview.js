...
import {
    closeDialogs, restoreDefaultEvents, fitContent, applySorting,
    addBurg, removeBurg, togglePort, toggleCapital,
    getFileName, downloadFile, uploadFile
} from "./editors.js";
...
const listStates = () => getById("burgsFilterState");
const listCultures = () => getById("burgsFilterCulture");
//const btnRefresh = () => getById("burgsOverviewRefresh");
//const btnRefresh = () => getById("burgsOverviewRefresh");
//const btnRefresh = () => getById("burgsOverviewRefresh");
//const btnRefresh = () => getById("burgsOverviewRefresh");
...
export function overviewBurgs() {
    ...
    // add listeners
    btnRefresh().addEventListener("click", refreshBurgsEditor);
    btnViewChart().addEventListener("click", showBurgsChart);
    listStates().addEventListener("change", burgsOverviewAddLines);
    listCultures().addEventListener("change", burgsOverviewAddLines);
...
function updateFilter() {
    ...
    const cultureFilter = listCultures();
...
function burgsOverviewAddLines() {
    const selectedState = +listStates().value;
    const selectedCulture = +listCultures().value;
    ...
    for (const b of filtered) {
        ...
        <input data-tip="Burg name. Click and type to change" class="burgName" value="${b.name}" autocomplete="off" spellcheck="false">
...
function changeBurgPopulation() {
    ...
    const population = [];
    getBody().querySelectorAll(":scope > div").forEach(el => population.push(+getInteger(el.dataset.population)));
...
function exitAddBurgMode() {
    ...
    clearMainTip();
    if (addBurgTool.classList.contains("pressed"))
        addBurgTool.classList.remove("pressed");
    if (addNewBurg.classList.contains("pressed"))
        addNewBurg.classList.remove("pressed");
}
