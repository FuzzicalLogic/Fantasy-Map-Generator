...
import { editLabel } from "./labels-editor.js";
...
export function addBurg(point) {
    ...
    const population = Math.max((cells[cell].s + cells[cell].road) / 3 + i / 1000 + cell % 100 / 1000, .1);
...
export function moveBurgToGroup(id, g) {
...
export function toggleCapital(burg) {
...
export function togglePort(burg) {
...
function createPicker() {
  ...
  <label>HEX: <input type="text" id="pickerHEX" data-space="hex" style="width:42px" autocomplete="off" spellcheck="false" value="#7d8ee8"></label>`;
...
export function uploadFile(el, callback) {
...
export function selectIcon(initial, callback) {
    ...
    $("#iconSelector").dialog({
        width: fitContent(), title: "Select Icon",
        buttons: {
